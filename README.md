...
基于炉石传说中文卡牌词库的多人实时“你画我猜”网页游戏。项目包含公共大厅、可配置房间、实时画板、十选一答题与候选卡牌搜索工具，并支持单人 AI 测试。
...
- 单人开局时自动加入“旅店老板 AI”，AI 与普通辅助共用彩色主体识别管线，按“完整椭圆卡面、主体彩色轮廓、分区铺色”顺序作画；卡图异常时自动使用类型草图兜底
- 多人实时 Canvas 画板、画笔、橡皮和清屏
- 可选辅助作画，按随从、法术、武器、英雄和地标版式裁剪插画，并以明暗、颜色和双尺度对比提取主体
- 答题提示按真实卡牌类型逐层拼装卡框、职业色、稀有度、费用和属性，插画保持空白；回合结算时直接替换为完整卡图
- 选择正确卡牌后即时生成 9 个同类型、同字数且属性相近的迷惑项，全部带卡图和编号
- 按名称、字数、费用、攻击、生命、耐久和护甲检索卡牌，支持分页
...
提示卡牌使用 HearthCards 第 5 版网页模板的坐标与素材，由 TypeScript 和 HTML Canvas 在浏览器中绘制，不依赖 Unity 或 Phaser。重新同步模板素材：

```powershell
npm run cards:templates
```

同步结果保存在 `public/hearthcards`，运行游戏时不依赖 HearthCards 外站。
...
| `FINAL_REVEAL_OVERRIDE_MS` | 未设置 | 测试用终局卡图提前量覆盖；生产默认最后 5 秒 |
